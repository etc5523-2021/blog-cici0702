...
In [Wikipedia](https://en.wikipedia.org/wiki/COVID-19_vaccination_in_Israel), Israel people get vaccinated very fast and get a high vaccination rate which has already reach the top highly vaccination rate countries per capita. But the four pandemic waves shows that although people get vaccination in Israel, it is unavoidable to get infected by Coronavirus. Let's see the distribution of different ages and genders.
...
The graph compares the male and female in Israel of being new COVID cases. Although there are some missing data here, it can clear see the four pandemic waves in Israel. Sometimes, the number of cases of female is larger than male.
...
map <- asia_vac %>%
  ...
  labs(fill = 'Fully vaccinated rate',
       color = 'Israel',
       title = 'Fully vaccinated rate of Covid in Asia',
       x = NULL,
       y = NULL)+
  geom_point(x = 30, y = 31.04,shape = 11, size = 3, color = "red")+
  annotate("text", x = 30, y = 25, label = "Israel")
...
+ Corona data by sex and age groups. Israel Government Databases(May 2020)
Retrieved from: https://data.gov.il/dataset/covid-19/resource/89f61e3a-4866-4bbf-bcc1-9734e5fee58e
...
+ Yihui Xie, Joe Cheng and Xianying Tan (2021). DT: A Wrapper of
  the JavaScript Library 'DataTables'. R package version 0.18.
  https://CRAN.R-project.org/package=DT
  
+ Ramnath Vaidyanathan, Kent Russell and Gareth Watts (2016).
  sparkline: 'jQuery' Sparkline 'htmlwidget'. R package version
  2.0.
